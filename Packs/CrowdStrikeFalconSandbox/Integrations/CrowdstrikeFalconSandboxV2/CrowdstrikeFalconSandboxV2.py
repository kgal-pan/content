...
def crowdstrike_scan_command(client: Client, args: Dict[str, Any]):
    ...
    def file_with_bwc_fields(res):
        return CommandResults(readable_output=create_scan_results_readable_output(res), indicator=BWCFile(res, {
            'sha1': 'SHA1', 'sha256': 'SHA256', 'md5': 'MD5', 'job_id': 'JobID', 'environment_id': 'environmentId',
            'threat_score': 'threatscore', 'environment_description': 'environmentDescription',
            'submit_name': 'submitname', 'url_analysis': 'isurlanalysis', 'interesting:': 'isinteresting',
            'vx_family': 'family'}, False, size=res.get('size'), file_type=res.get('type'), sha1=res.get('sha1'),
             sha256=res.get('sha256'), md5=res.get('md5'), sha512=res.get('sha512'), name=res.get('submit_name'),
...
def main() -> None:
    ...
    try:
        ...
        match demisto.command():
            case 'test-module':
                command = test_module
            case 'cs-falcon-sandbox-search' | 'crowdstrike-search':
                command = crowdstrike_search_command
            case 'cs-falcon-sandbox-scan' | 'crowdstrike-scan' | 'file':
                command = crowdstrike_scan_command
            case 'crowdstrike-get-environments' | 'cs-falcon-sandbox-get-environments':
                command = crowdstrike_get_environments_command
            case 'cs-falcon-sandbox-get-screenshots' | 'crowdstrike-get-screenshots':
                command = crowdstrike_get_screenshots_command
            case 'cs-falcon-sandbox-result' | 'crowdstrike-result':
                command = crowdstrike_result_command
            case 'cs-falcon-sandbox-analysis-overview':
                command = crowdstrike_analysis_overview_command
            case 'cs-falcon-sandbox-analysis-overview-summary':
                command = crowdstrike_analysis_overview_summary_command
            case 'cs-falcon-sandbox-analysis-overview-refresh':
                command = crowdstrike_analysis_overview_refresh_command
            case 'crowdstrike-submit-sample' | 'cs-falcon-sandbox-submit-sample':
                command = crowdstrike_submit_sample_command
            case 'cs-falcon-sandbox-submit-url' | 'crowdstrike-submit-url':
                command = crowdstrike_submit_url_command
            case 'cs-falcon-sandbox-sample-download':
                command = crowdstrike_sample_download_command
            case 'cs-falcon-sandbox-report-state':
